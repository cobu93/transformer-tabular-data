...
class AnnealTransformerConfig(TransformerConfig):
    ...
    def get_encoders(self, embedding_size, *args, **kwargs) -> List[FeatureEncoder]:
        return [
            ...
            CategoricalOneHotEncoder(embedding_size, 1),
            CategoricalOneHotEncoder(embedding_size, 2),
            CategoricalOneHotEncoder(embedding_size, 50),
            CategoricalOneHotEncoder(embedding_size, 68),
            CategoricalOneHotEncoder(embedding_size, 24),
            CategoricalOneHotEncoder(embedding_size, 2),
            CategoricalOneHotEncoder(embedding_size, 4),
            CategoricalOneHotEncoder(embedding_size, 3)
            ]
    ...
    def get_aggregator(self, embedding_size, *args, **kwargs) -> BaseAggregator:
        return CLSAggregator(embedding_size)

    
    def get_preprocessor(self, *args, **kwargs) -> BasePreprocessor:
        return CLSPreprocessor()

class AnnealSearchSpaceConfig(SearchSpaceConfig):
...
class AnnealDatasetConfig(DatasetConfig):

    def __init__(self):
        self.dir_name = "anneal/data"
        self.columns = None
