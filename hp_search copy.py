...
sweep_configuration = {
    ...
    "parameters": {
        ...
        "optimizer__lr": {"values": [1e-3, 5e-4, 1e-4, 5e-5]},
        "optimizer__weight_decay": {"values": [5e-4, 5e-5]},
        "dropout": {"values": [0., 0.2, 0.4]},
        "n_head": {"values": [1, 2, 4, 8, 16, 32]},
        "n_hid": {"values": [32, 64, 128, 256, 512]},
        "attn_dropout": {"values": [0, 0.1, 0.2, 0.3]},
        "ff_dropout": {"values": [0, 0.1, 0.2, 0.3]},
        "embed_dim": {"values": [32, 64, 128, 256]},
        "numerical_passthrough": {"values": [True, False]}
    }
}

sweep_configuration_rnn = {
        "aggregator__cell": {"values": ["LSTM", "GRU"]},
        "aggregator__hidden_size": {"values": [32, 64, 128, 256, 512, 1024]},
        "aggregator__num_layers":  {"values": [1, 2]},
        "aggregator__dropout":  {"values": [0, 0.1, 0.2]}
}
...
"""
Train a single model for a single fold here
"""
#def train(fold_name, sweep_id, sweep_run_name, config):
def train(dataset, fold_name, sweep_id, sweep_run_name, config):
...
def cross_validate():
    ...
    # Sweep creation and configuration
...
def main():
    wandb.login()
    sweep_id = wandb.sweep(sweep_configuration, project="tabular_transformer")
