...
def main():
    ...
    executions_df = reporting.load_report_df(
        jobs=LARGE_WORKER_JOBS,
        architectures_filename=os.path.join(DATA_BASE_DIR, "large_architectures.json"),
        folds_names=[f"F{i}" for i in range(K_FOLD)]
    )
    executions_cv_df = reporting.get_executions_cv_score(executions_df, arch_cols=["architecture_name", 
                        "attn_dropout", 
                        "embed_dim", 
                        "ff_dropout", 
                        "n_head", 
    ...
    executions_cv_df.to_csv("large_cross_validation_scores.csv", index=False)
    ...
    archs_file = os.path.join(DATA_BASE_DIR, "large_architectures.json")
    ...
    for job in best_archs_df.iloc:
        ...
        for t_training in range(TEST_TRAININGS):
            ...
            results, checkpoint_dir = refit(
                ...
                dataset_meta,
                config=arch,
                best_tag=f"large/{arch_name}/{aggregator}"
            )
    ...
    best_archs_df.to_csv("large_selected_architectures.csv", index=False)        
